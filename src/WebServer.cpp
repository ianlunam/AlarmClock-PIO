...
String alarmToTableRow(AlarmEntry &thisAlarm)
{
    ...
    output.concat("</td><td>");
    if (thisAlarm.hour < 10)
        output.concat("0");
    output.concat(thisAlarm.hour);
    output.concat(":");
    if (thisAlarm.minute < 10)
        output.concat("0");
...
void handleSave(AsyncWebServerRequest *request)
{
    ...
    if (request->hasArg("monday"))
    {
        alarmEntry.monday = true;
    }
    if (request->hasArg("tuesday"))
    {
        alarmEntry.tuesday = true;
    }
    if (request->hasArg("wednesday"))
    {
        alarmEntry.wednesday = true;
    }
    if (request->hasArg("thursday"))
    {
        alarmEntry.thursday = true;
    }
    if (request->hasArg("friday"))
    {
        alarmEntry.friday = true;
    }
    if (request->hasArg("saturday"))
    {
        alarmEntry.saturday = true;
    }
    if (request->hasArg("skip_phols"))
    {
        alarmEntry.skip_phols = true;
    }
    if (request->hasArg("once"))
    {
